...
def load_monodepth2_model(channels_per_image_depth, channels_per_image_pose, method='monodepth'):
    if method == 'monodepth':
        ...
        import networksIID
        print("   Loading pretrained encoder")
        encoder = networksIID.ResnetEncoder(18, False)
        print("   Loading pretrained decoder")
        depth_decoder = networksIID.DepthDecoder(
            num_ch_enc=encoder.num_ch_enc, scales=range(4))
...
def process_single_model(model_path, input_dir, output_root, ext='png', target_resolution=(1440, 1080), method='monodepth'):
    ...
    with torch.no_grad():
        for idx, image_path in enumerate(image_paths):
            ...
            print(f"Folder: {folder_name}, Frame Index: {frame_index}, Use Edge: {use_edge}, Use Lum: {use_lum}")
...
# iid-sfm_hk_interval20           monovit_hkfull_mono_finetuned
    ...
    method = "monovit"  # Default method, can be overridden by model path
    # Process each model
    for i, model_path in enumerate(model_list):
        ...
        try:
            process_single_model(model_path, input_dir, output_root, target_resolution=(1440, 1080), method = method)
